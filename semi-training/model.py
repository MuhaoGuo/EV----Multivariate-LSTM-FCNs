...
class MLSTMfcn(nn.Module):
    ...
    def forward(self, x, seq_lens):
        ...
        # x1 = nn.utils.rnn.pack_padded_sequence(x, seq_lens, batch_first=True, enforce_sorted=False)    # 先填充后压缩
        x1, (ht, ct) = self.lstm(x)
        # x1, _ = nn.utils.rnn.pad_packed_sequence(x1, batch_first=True, padding_value=0.0)     # 压缩后填充
        x1 = x1[:, -1, :]
